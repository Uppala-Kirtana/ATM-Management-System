            ...
            <div class="row">
                ...
                <div class="col" style="text-align: center;justify-content: center; align-items: center; display: flex; padding-top: 30px; padding-bottom: 30px;">
                    <form action="" method="post">
                        <label for="">
                        <h3 style="font-family: 'Times New Roman', Times, serif;">Enter Old Pin</h3>
                        </label><br>
...
$np = $_POST['np2'];
$cnp = $_POST['np3'];
if (!filter_var($an, FILTER_VALIDATE_INT)) {
    echo 'Swal.fire({
        icon: "error",
...
elseif (!filter_var($np, FILTER_VALIDATE_INT)) {
    echo 'Swal.fire({
        icon: "error",
        title: "Only Integers are allowed",
        text: "Please try again."
...
elseif (!filter_var($cnp, FILTER_VALIDATE_INT)) {
    echo 'Swal.fire({
        icon: "error",
        title: "Only Integers are allowed",
        text: "Please try again."
...
elseif ($np === $cnp) {
    if (strlen($np) <= 4) {
        ...
        $qry = mysqli_query($conn, "SELECT * FROM u_login WHERE status='1' AND BINARY u_pin='$an' AND u_id='$mainid'");
        ...
        if ($qry && mysqli_num_rows($qry) > 0) {
            ...
            if ($np !== $currentPin) {
                ...
                if ($uqry) {
                    ...
                    echo 'Swal.fire({
                        icon: "error",
                        title: "Pin Error",
                        text: "Error updating pin. Please try again."
                    });';
                }
            } else {
                echo 'Swal.fire({
            ...
            echo 'Swal.fire({
                icon: "error",
                title: "Old pin number is incorrect",
                text: "Please try again."
            });';
        }
    } else {
