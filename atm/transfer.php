    ...
    <div style="padding-left:20px;"><h4>User Name: <?php echo isset($a4) ? $a4 : ''; ?></h4></div>
        ...
        <div class="container row-cols-1 text-center" style="border: 2px solid black;">
            <div class="row">
                ...
                <label for="">
                        <h3 style="font-family: 'Times New Roman', Times, serif;"><b>Your Account Balance is: <?php echo isset($accBalance) ? $accBalance : ''; ?></b></h3>
                    </label><br>
                ...
                <label for="">
                        ...
                        <h4 style="font-family: 'Times New Roman', Times, serif;">Enter Transfer Amount</h4>
                        </label>
                        <input type="text" class="form-control" id='w1' name='w1' placeholder="Enter Transfer Amount" style="width:250px;height:50px;" /><br>
...
<script>
    ...
    if (isset($_POST['proceed'])) {
        ...
        elseif ($wd <= 0 || $wd > $accBalance) {
            ...
            $insertAccQuery = mysqli_query($conn, "INSERT INTO u_acc (u_id, ftransfer) VALUES ('$storedMainid', '$wd')") or die(mysqli_error($conn));
    
            $updateLoginQuery = mysqli_query($conn, "UPDATE u_login SET acc_bal='$newBal' WHERE u_id='$storedMainid'") or die(mysqli_error($conn));
            
            $qrysel = mysqli_query($conn, "SELECT acc_bal FROM u_login WHERE status='1' AND u_acc='$b1'") or die(mysqli_error($conn));
            ...
            $rowBeneficiary = mysqli_fetch_assoc($qrysel);
            $nb = $rowBeneficiary['acc_bal'];
            $nb2=$nb+$wd;
            
            $updateBeneficiaryQuery = mysqli_query($conn, "UPDATE u_login SET acc_bal='$nb2' WHERE u_acc='$b1'") or die(mysqli_error($conn));
    
            if ($insertAccQuery && $updateLoginQuery && $updateBeneficiaryQuery) {
                echo 'Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "loadft.php";
                    }
                });';
